...
/* @Prototype */
{
	...
	get_carenets: function(document_id, success, error) {
		...
		console.log(document_id.substring(9))	
		if (document_id.indexOf('companion')!= -1){
			
			return $.ajax({
                        url: this.baseURL() + '/shares/',
                        type: 'post',
                        dataType: 'carenet.models',
                        data:{'account_id':document_id.substring(9)},
                        success: success,
                        error: error
                });
	...
	create_share: function(success,error){

		 $.ajax({
                        type: 'post',
                        url: '/records/' + this.id + '/shares/',
