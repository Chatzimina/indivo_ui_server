...
/* @Prototype */
{
    ...
    didReloadRecord: function(record) {
    	...
    	this.carenets = [];
	this.companions = [];
//        this.record.get_carenets('companion', this.callback('didLoadCarenets'));
			...
			if (c_accounts && c_accounts.length > 0) {
				...
				for (var i = 0; i < c_accounts.length; i++) {		// can't use "concat" here as this wraps the accounts into some jquery element, stupidly :P
					if (!_(this.accounts).detect(function(a) { return a.id === c_accounts[i].id; })) {
						...
						this.companions.push(c_accounts[i]);
					}
				}
			}
			else {
...
//	this.view('accounts', {'accounts': this.accounts});
//	 $('#known_accounts').html(this.view('accounts', {'record': this.companions}));
		...
		$('#known_accounts').find('.companion.new').click(function(ev) {
                        self.showNewCompanionBox(ev, $(this));
                });
	 ...
	 */
	 ...
	 showNewCompanionBox: function(ev, el, error_msg) {
                var self = this;
                ...
                form.find('.companion_buttons img').remove();
                form.find('.companion_buttons button').show();
                ...
                form.unbind('submit').bind('submit', function(ev) {
                        ...
                        var name_field = form.find('input.companion_name');
                        ...
                        var known_accounts = $('#known_accounts').find('.companion').not('.new');
                ...
                el.find('div.companion_name').hide();
                form.show();
                form.find('input.companion_name').focus();
                form.find('button[type="reset"]').click(function(ev) {
                        ev.stopPropagation();
                        self.hideNewCompanionBox();
                });
        },
	...
	hideNewAccountBox: function() {
	 ...
	 hideNewCompanionBox: function() {
                var form = $('#new_companion_form');
                form.parent().removeClass('highlight');
                form.hide().siblings().show();
        },
	...
	getAccountName: function(account_id) {
        ...
        getCompanionName: function(account_id) {

                if (account_id) {
                        var new_companion = new UI.Models.Account({'id': account_id});
                        global_account=new_companion;

                        new_companion.get_name(this.callback('didGetCompanionName', new_companion));
                }
        },
...
//var url = '/indivoapi/accounts/johnsmith@indivo.org';
//alert(url);
//		AjaxRequest(url, 'POST', callapi);




//DEN EIMAI SIGOURH AN XREIAZETAI .mariaa
/*var url = ''; //'https://www.iphr.care/indivoapi/accounts/johnsmith@indivo.org';
	$.ajax({
   url: url,
   ...
   console.log('data  = ' + response.toSource());
   },
   error: function(response) {
  console.log('ERROR '+response);
  console.log('error data = ' +response.toSource());
...
if ('success' != textStatus) {
                      ...
                      $.ajax({
                      type: 'POST',
                      url: "https://www.iphr.care/sendEmailNewUser?contact_email="+account['id'],
                      success: function(response){
                                  output = response;
				  alert(response)
                                  alert("An email has been sent to "+account['id']+" in order to register to the portal.");
                                },
                      async: false
                      });
...
//			$.get("/jmvc/sendmail.cgi");
//			 $.ajax({
//			      type: 'POST',
//			      url: '/jmvc/sendmail.cgi',
...
//			      });
	...
	didGetCompanionName: function(account, data, textStatus, xhr) {
...
//                            });
                        ...
                        this.showNewCompanionBox(null, $('#new_account_form').parent(), data);
                }
                else {
			this.record.get_carenets('companion'+account['id'], this.callback('test',account['id'],data));
...
//                        this.showAccounts();
	...
	test : function(account,data){
		var text = document.getElementById("text").innerHTML;

		alert('Companion was added successfully.');
		text+=' <div class="account" > <div class="account_name">'+data+'</div> <div class="account_id">'+account+'<br><input id="delete" type="button" value="Delete" onclick="deleteShare('+"'"+account+"'"+');" /></div> </div>';
		 $("#resetbutton").click();
