...
/* @Prototype */
{
		...
		// ** load demographics first and call us again when we're all good
			...
			record.get_demographics(
				...
				function(data, textStatus, xhr) {
                                  ...
                                  if (width > 800){
					self.fillDemographicsArea();  //maria homepage startpage
					}
				},
	...
	fillDemographicsArea: function() {
		...
		$('#demographics_area').empty().append($.View('//ui/views/record/demographics', {'demographics': demographics,'record':record}));
	},
	...
	showDemographicsForm: function() {
		...
		// load the form
		var form = $.View('//ui/views/record/demographics_form', {'demographics': demographics,'record':record});
		ui_main.tintInterface();
	...
	'#demographics_form submit': function(element, event) {
		...
		var demographics = '<Demographics xmlns="http://indivo.org/vocab/xml/documents#">'
                                    ...
                                    +'</Name>'
                                    +'<siop>' + element.find('input[name="siop"]').val() + '</siop>'
                                    +fon1
                                    +fon2
                                    +address
                                +'</Demographics>';
	...
	'#new_record_form submit': function(el, ev) {
		...
		// collect data (where's that "form_values" method of jQuery?)
		if(!el.find('input[name="outputtext"]').val()){
		var demographics = '<Demographics xmlns="http://indivo.org/vocab/xml/documents#">'
                                  +'<dateOfBirth>' + el.find('input[name="year"]').val() + '-' + el.find('select[name="month"]').val() + '-' + el.find('select[name="day"]').val() +'</dateOfBirth>'
                                   +'<gender>' + el.find('select[name="sex"]').val() + '</gender>'
                                    ...
                                    +'</Name>'
                                    +'<siop>' + el.find('input[name="siop"]').val() + '</siop>'				
                                +'</Demographics>';
		         alert('Record created. Please refresh page.')
			 UI.Models.Record.create(demographics, this.callback('didCreateNewRecord', el), this.callback('didNotCreateNewRecord', el));

		  }
		  if(el.find('input[name="outputtext"]').val()){
                  var demographics = el.find('input[name="outputtext"]').val();	
			alert("Please refresh page")
 		  var test1 = demographics.split( "\n" );
		  for(i=0; i<=test1.length; i++){
			
			var lineSplit = test1[i].split("\t");
			if(lineSplit[0]!=""){
			var demographics = '<Demographics xmlns="http://indivo.org/vocab/xml/documents#">'
					   +'<dateOfBirth>'+lineSplit[0]+'</dateOfBirth>'
					    +'<gender>'+lineSplit[1]+'</gender>'
					     +'<email>'+lineSplit[2]+'</email>'
						+'<Name>'
						+'<familyName>'+lineSplit[3]+'</familyName>'
						+'<givenName>'+lineSplit[4]+'</givenName>'
					        +'</Name>'
                                                +'<siop'+lineSplit[5]+'</siop>'
					    +'</Demographics>';
