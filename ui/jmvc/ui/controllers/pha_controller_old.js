...
/* @Prototype */
{
	...
	didGetAllApps: function(all_apps) {			// got all apps, get my apps
		...
		$.ajax({
  			type: "POST",
  			url: "/jmvc/ui/controllers/enableApp.py",
  			data: { param: this.record.id}
		...
		//$.ajax({
        	...
        	//	type: 'POST',
        	//	url: '/jmvc/ui/controllers/enableApp.py',
        	//	data: this.record, // <-- This one is changed into an object
    		//	success: function (response) {
        	//		alert(response); // <-- Append the ajax response to the page body
    		//	},
	...
	didGetMyApps: function(my_apps) {			// got my apps, get carenets we are in
		...
		var req = new XMLHttpRequest();
                 req.open('GET', '/jmvc/ui/controllers/apps.json', false);
                 req.send(null);
                 var testFile = req.responseText;

		 data = testFile;

		    //#appsData = json.loads(f)

    		

		 var appsJSON = JSON.parse(data);
		 //alert(someData_notJSON[0].red);  
//		 var request = new XMLHttpRequest();
//		 request.open("GET", "jmvc/ui/controllers/apps.json", false);
//		 request.send(null)
//
 // 		 var my_JSON_object = JSON.stringify(request.responseText);
   		// alert (appsJSON[0].Apps);
		 //appsJSON[0].Apps = 't';		 
		 //var data2 = new FormData();
		 //data2.append("data" , "the_text_you_want_to_save");
		 //var xhr = (window.XMLHttpRequest) ? new XMLHttpRequest() : new activeXObject("Microsoft.XMLHTTP");
		 //xhr.open( 'post', '/jmvc/ui/controllers/maria.txt', true );
		 //xhr.send(data);	         
		// var dataTest = "maria";// this is your data that you want to pass to the server (could be json)
	...
	//	}	
		...
		for (var i = 0; i < this.all_apps.length; i++) {
			var app = this.all_apps[i];
                        ...
                        if (app.app_id == 'labs@apps.indivo.org'){
