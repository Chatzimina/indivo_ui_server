...
def get_connect_credentials(request, account_id, app_email):
    ...
    resp, content = api.get_connect_credentials(account_email=account_id, pha_email=app_id, body=data)
...
def index(request):  #MERGE
    ...
    try:
       r = requests.get("http://thor.ics.forth.gr:8580/ProfilingServer/getPatientResults?clinicianKey=clinician12.&patientId=396i@gqr!a", timeout=0.00001)
   ...
   #    print e
    ...
    if tokens_p(request):
        ...
        return utils.render_template('ui/index', { 'ACCOUNT_ID': account_id,
                                                    'SETTINGS': settings, 'jsonData': jsonData})
