...
##    if not request.session.has_key('procedures_access_token'):
    ...
    if 1==2:
       if datetime.datetime.now() > request.session['procedures_time'] + datetime.timedelta(minutes=5):
    ...
    # store the request token in the session for when we return from auth
         ...
         access_token=request.session['procedures_access_token']
...
def after_auth(request):
    ...
    if token_in_session['oauth_token'] != oauth_token:
        return HttpResponse("oh oh bad token please refresh page")
    ...
    request.session['procedures_access_token']=access_token
    request.session['procedures_time']=datetime.datetime.now()
#    access_token={}
...
#        if r != " ":
 ...
 #          jsonData = " "
    ...
    return render_template('list', {'record_label': record_label, 'num_problems' : num_problems, 
                                     'problems': probs, 'in_carenet':in_carenet, 'jsonData':jsonData,'record_id':record_id,'ids':ids,'surl_credentials':surl_credentials })
...
#      name_type = (name_type2[:18] + '') if len(name_type2) > 19 else name_type2
       name_type=request.POST['code_fullname']
       name_abbrev2 = request.POST['code']
       name_abbrev = (name_abbrev2[:18] + '') if len(name_abbrev2) > 19 else name_abbrev2

       # get the variables and create a problem XML
       params = {'date_performed': date_performed, 
                  'name': 'http://purl.bioontology.org/ontology/SNOMEDCT/',#request.POST['code_fullname'], 
                  'name_type': name_type,#'http://purl.bioontology.org/ontology/SNOMEDCT/', 
                  'name_abbrev':name_abbrev,
                  'name_value': 't', #request.POST['comments_code'], 
                  'provider_name': request.POST['provider_name'], 
                  'provider_institution': request.POST['provider_institution'], 
                  'location': request.POST['location'], 
                  'comments' : request.POST['comments']}
